...
class PicoHarp300(FastCounterInterface):
    ...
    def _set_constants(self):
        ...
        self.DISCRMIN = 0
        self.DISCRMAX = 800
        self.PHR800LVMIN = -1.6
        self.PHR800LVMAX = 2.4
    ...
    def set_input_CFD(self, channel, level, zerocross):
        ...
        if channel not in (0, 1):
            self.log.error('PicoHarp: Channel does not exist.\nChannel has '
    ...
    def start(self, acq_time):
        ...
        if not(self.ACQTMIN <= acq_time <= self.ACQTMAX):
            ...
            self.check(self._dll.PH_StartMeas(self._deviceID, int(acq_time*1e3)))
    ...
    def set_phr800_input(self, channel, level, edge=0):
        ...
        @param int level: set the trigger voltage level in V. The entered
        ...
        if not(self.PHR800LVMIN <= level <= self.PHR800LVMAX):
            self.log.error('PicoHarp: Invalid level for routing.\n'
                           ...
                           '[{1},{2}] V, but a value of {3} was passed.'
                           ''.format(channel, self.PHR800LVMIN, self.PHR800LVMAX, level))
            return
        if (int(edge) != 0) and (int(edge) != 1):
        ...
        self.check(self._dll.PH_SetPHR800Input(self._deviceID, channel, int(level*1000), int(edge)))
    ...
    def get_constraints(self):
        ...
        return {'hardware_binwidth_list': [2e-9]}
