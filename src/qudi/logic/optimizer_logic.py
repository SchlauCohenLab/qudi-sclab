...
from qudi.util.fit_models.gaussian import Gaussian2D, Gaussian, GaussianLinear
...
class OptimizerLogic(LogicBase):
    ...
    def _set_optimized_xy_from_fit(self):
        ...
        xy = np.meshgrid(self._X_values, self._Y_values)
        xy_fit_data = self.xy_refocus_image[:, :, 3+self.opt_channel].ravel()
        # print(result_2D_gaus.fit_report())
        opt_pos, fit_data, fit_res = self._get_pos_from_2d_gauss_fit(self, xy, xy_fit_data)
        if fit_data is None:
            ...
            if abs(self._initial_pos_x - fit_res.best_values['center_x']) < self._max_offset and abs(self._initial_pos_x - fit_res.best_values['center_x']) < self._max_offset:
                if self.x_range[0] <= fit_res.best_values['center_x'] <= self.x_range[1]:
                    if self.y_range[0] <= fit_res.best_values['center_y'] <= self.y_range[1]:
                        self.optim_pos_x = fit_res.best_values['center_x']
                        self.optim_pos_y = fit_res.best_values['center_y']
                        self.optim_sigma_x = fit_res.best_values['sigma_x']
                        self.optim_sigma_y = fit_res.best_values['sigma_y']
            else:
                self.optim_pos_x = self._initial_pos_x
                self.optim_pos_y = self._initial_pos_y
                self.optim_sigma_x = 0.
